...
export default function AssemblyModal({ assembly, onClose, onSave, onDelete }) {
        ...
        <div className="flex items-center justify-between px-10 py-8 border-b border-[var(--border-color)] bg-[var(--bg-main)]">
          <div>
            <h2 className="text-2xl font-black text-[var(--text-primary)] tracking-tighter uppercase">
              Member <span className="text-[var(--accent)]">Detail</span>
            </h2>
            <p className="text-xs font-bold text-[var(--text-secondary)] uppercase tracking-[0.15em] mt-1 opacity-60">Assembly Configuration</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-10 py-8 space-y-8 custom-scrollbar">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-[var(--bg-main)] p-6 rounded-3xl border border-[var(--border-color)]">
            <Field label="Part Number">
              <div className="font-mono-eng text-base text-[var(--text-primary)] font-black tracking-tight flex items-center gap-2 bg-[var(--bg-card)] px-4 py-2 rounded-2xl border border-[var(--border-color)]">
                <span className="w-2 h-2 rounded-full bg-[var(--accent)]" />
                {assembly.partno}
              </div>
            </Field>
            <Field label="Job Identity">
              <div className="font-mono-eng text-sm text-[var(--text-primary)] font-bold opacity-80 bg-[var(--bg-card)] px-3 py-1.5 rounded-xl border border-[var(--border-color)] inline-block">
              ...
              {!showAdd ? (
                <div className="flex gap-3">
                  ...
                  >
                    ...
                    {category && !PREDEFINED_CATEGORIES.includes(category) && (
